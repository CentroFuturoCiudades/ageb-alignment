...
def framework_factory(geo_type: Literal["state", "mun", "loc"], year: int) -> dg.AssetsDefinition:
    @dg.asset(
        name=str(year),
    ...
    def _asset(
        ...
        if geo_type == "state":
            census = census.assign(CVEGEO=lambda df: df["CVEGEO"].astype(str).str.zfill(2))
        elif geo_type == "mun":
            census = census.assign(CVEGEO=lambda df: df["CVEGEO"].astype(str).str.zfill(5))
        elif geo_type == "loc":
